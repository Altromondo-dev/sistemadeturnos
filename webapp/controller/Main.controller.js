...
sap.ui.define([
	...
	"transener/sistemadeturnos/services/RegionesService",
	"transener/sistemadeturnos/services/PersonalHabilitadoService",
	"transener/sistemadeturnos/services/WorkPlaceService",
	"transener/sistemadeturnos/services/oDataService",
	"transener/sistemadeturnos/services/EmpresaTramitacionService",
	"transener/sistemadeturnos/services/TipoEquipoService",
	"transener/sistemadeturnos/services/EquiposService",
	"transener/sistemadeturnos/services/OrdenesService",
	"transener/sistemadeturnos/services/EstacionesService",
	"transener/sistemadeturnos/services/RepositionTimeService",
	...
	//excel
	"transener/sistemadeturnos/view/Main/ExcelDownloadHelper",
	...
	"transener/sistemadeturnos/utils/RolAuthorizationHelper",
...
], function (Controller, Fragment, Filter, FilterOperator, TableRow, MessageToast, NavigationHelper, FormatHelper, FioriComponentHelper, MailHelper,
	ValidateHelper,
	...
	UserService, ExcelDownloadHelper, BusyDialogHelper, FioriHelper, LicenseHelper, RolAuthorizationHelper, FormatterHelper,
	...
	return Controller.extend("transener.sistemadeturnos.controller.Main", {
		...
		setApplicationModels: function () {
			...
			this.getView().setModel(models.createFiltersModel(), "FiltersJsonModel");
		...
		_routePatternMatched: function (oEvent) {
			...
			if (aRoles.indexOf("Visualizador") == -1) {
				...
				this.validURLToLicense = LicenseHelper.validURLToLicense(sKey);
		...
		onSearch: function () {
			...
			var sFromDate = dateInitPicker.getDateValue()
			var sToDate = dateEndPicker.getDateValue()
		...
		onShowDetailsPress: function (oEvent) {
			...
			if (oData.expanded) {
				// Añadir filas anidadas
				oData.nestedData.forEach((nestedItem, index) => {
					aData.splice(iIndex + index + 1, 0, {
						nested: true,
						...nestedItem
					});
				});
			} else {
				// Eliminar filas anidadas
				oData.nestedData.forEach(() => {
					aData.splice(iIndex + 1, 1);
				});
			}
		...
		goToEdit: function (oEvent, bManualPress) {
			...
			LicenseHelper.generateDeliveryDevolution(oLicense);
		...
		openMassiveTramitationAddCompanyDialog: function () {
			...
			if (this.checkIfAllAreLicences(selectedItemsSinFiltrar)) {
				if (this.getMassiveTramitationsAvailability(selectedItemsSinFiltrar) === false) {
					...
					var pageName = "transener.sistemadeturnos.views.Main.Dialogs.MassiveTramitation";
		...
		_tramitarMasivamente: function (aLicencias, aTramitaciones) {
			...
			aLicencias.forEach(oLicencia => {
				...
				oLicencia.Licstat = LicenseHelper.getTramitStatus(aTramitaciones);
				oLicencia.Timbeg = new Date(oLicencia.Timbeg.ms);
				oLicencia.Timend = new Date(oLicencia.Timend.ms);
				let oLicenciaClone = LicenseHelper.cloneLicense(oLicencia);
		...
		onClearFilter: function () {
			...
			var oLicenseTableItems = oTable.getBinding("rows");
			var aFilter = [];

			oLicenseTableItems.filter(aFilter);
		},
		...
		_adjustShiftForLicense: function (aLicences, iLicenseIndex, minutesToAdd) {
			...
			// Asignar el turno actual sin cambiarlo para la licencia seleccionada
			aLicences[iLicenseIndex].TurnoAsignado = this._formatTime(currentTime);

			// Reasignar turnos para las licencias siguientes
			for (var i = iLicenseIndex + 1; i < aLicences.length; i++) {
				// Si el grupo cambia, agregar minutos al tiempo actual
				if (aLicences[i].Grupo !== aLicences[i - 1].Grupo) {
					currentTime = this._convertShiftToMinutes(aLicences[i].TurnoAsignado);
					currentTime += minutesToAdd; // Agregar los minutos especificados al tiempo actual
				}

				// Asignar el turno a la licencia actual
				aLicences[i].TurnoAsignado = this._formatTime(currentTime);
			}
