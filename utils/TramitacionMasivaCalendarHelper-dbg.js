sap.ui.define([
	...
	return {
						...
						items: [
							...
							new sap.m.Table({
								...
								items: {
									...
									path: "TramitacionMasivaListJsonModel>" + this._oModelPath + "/CalendarDates",
									template: new sap.m.ColumnListItem({
										cells: [
											new sap.m.Text({
												text: {
													path: "TramitacionMasivaListJsonModel>Fecha",
											...
											new sap.m.Text({
												text: "{TramitacionMasivaListJsonModel>Observaciones}"
											}),
											new sap.m.HBox({
												items: [
													new sap.m.Button({
		...
		replaceDate: function (oDatePayload) {
			var oModelTramitacion = this._oDialog.getModel("TramitacionMasivaListJsonModel")
			...
			if (this._bHasTraindex) {
				...
				LicenseService.getCalendarPostTramitacion(oPayloadTramitacion, oDatePayload, oDatePayload.Fecha).then(() => {
					LicenseService.getDatesFromTramitacion(oPayloadTramitacion).then((data) => {
						...
						this._oDialog.getModel("TramitacionMasivaListJsonModel").setProperty(this._oModelPath + "/CalendarDates", data.results)
		...
		_deleteCalendarDate: function (oEvent) {
			...
			var oControl = event.getBindingContext("TramitacionMasivaListJsonModel").getObject();
			MessageBoxHelper.showConfirm("Alerta", "¿Está seguro que desea eliminar esta fecha?", () => {
				if (oControl.Traindex) {
					...
					var oModel = this._oDialog.getModel("TramitacionMasivaListJsonModel")
		...
		closeTramitacionCalendar: function () {
			this._oDialog.close();
